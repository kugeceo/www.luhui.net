(function (w, d, PKAE) {
		...
		return [
						...
						action: function () {
							new PKSimpleModal({
								...
								body: '<div class="pk_row"><label for="k0">文件名</label>' +
									...
									'<label class="pk_lblmp3" for="k5">仅导出选中部分</label></div>',
...

					{
						...
						action: function (e) {
							new PKSimpleModal({
								...
								body: '<label for="k00">URL地址</label>' +
									'<input style="min-width:250px" placeholder="请输入URL地址" class="pk_txt" type="text" id="k00" />',
								setup: function (q) {
						...
						action: function (e) {
							...
							new PKSimpleModal({
								...
								body: '<p>请选择目标文件</p>' +
									...
									'<label style="vertical-align:top" for="sl1">整个音轨</label>' +
									'<input type="radio" class="pk_check"  id="sl2" name="rdslnc" value="sel">' +
									'<label style="vertical-align:top" class="pk_lblsel" for="sl2">仅选中部分' +
									'<i style="display:block;font-size:11px;margin-top:-5px"></i></label>' +
									'<input type="radio" class="pk_check"  id="sl3" name="rdslnc" value="copy">' +
									'<label style="vertical-align:top" class="pk_lblsel2" for="sl3">剪切板</label></div>' +
									'<div class="pk_row"><label for="slk0">缓存文件名</label>' +
									'<input style="min-width:250px" placeholder="（可选）请输入文件名" maxlength="100" ' +
										...
										app.fls.DelSession(name, function (name) {
											...
											if (el) {
												if (el.parentNode.children.length === 1) {
													el.parentNode.innerHTML = '没有找到缓存文件';
												}
			...
			},
			{
				...
				children: [
					...
					{
						name: '恢复 <span class="pk_shrtct">Shft+Y</span>',
						clss: 'pk_inact',
						action: function () {
					...
					{
						name: '播放 <span class="pk_shrtct">Space</span>',
						action: function () {
					...
					{
						name: '取消全选 <span class="pk_shrtct">~</span>',
						action: function () {
			...
			},
			{
				...
				children: [
					...
					{
						name: '居中光标 <span class="pk_shrtct">[Tab]</span>',
						action: function (obj) {
					...
					{
						name: '重置指纹 <span class="pk_shrtct">[0]</span>',
						action: function (obj) {
			...
			},
			{
				...
				children: [
...

					{
						name: '关于本站',
						action: function () {
							window.open('http://www.luhui.net/wap/about.html');
						}
					},
...

					{
						name: '鲁虺音乐',
						action: function () {
							window.open('http://music.luhui.net');
						}
					}
				]
			}
	...
	function _makeUIToolbar(UI) {
		...
		copy_btn.innerHTML = '<span>复制选中项 (Shift + C)</span>';
		...
		paste_btn.innerHTML = '<span>粘贴选中项 (Shift + V)</span>';
		...
		cut_btn.innerHTML = '<span>剪切选中项 (Shift + X)</span>';
		...
		btn_clear_selection.innerHTML = '<span>重置选项 (按键Q)</span>';
