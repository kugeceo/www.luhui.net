...
function setCookie(name,value,time){
  ...
  var expireStr=time>0?' expires='+(new Date(new Date().getTime()+(time*24*60*60+timePos*3600)*1000).toGMTString())+';':'';
  /*
...
function addM(obj) {
	if (getCookie('usercookie') != false) {
		var starName = obj.innerHTML;
		var starHttp = obj.href;
		document.getElementById('addPFrame').src='member.php?post=memory_website&linkhttp=' + encodeURIComponent(starHttp) + '&linkname=' + encodeURIComponent(starName) + '';
		//window.open('member.php?post=memory_website&linkhttp=' + encodeURIComponent(starHttp) + '&linkname=' + encodeURIComponent(starName) + '', 'addPFrame')
	}
}

function addPoint(rf) {
	if (getCookie('yourpcaddfunds') == false) {
		var runAdd = false;
...
function addLoadElement() {
	addPoint(document.referrer);
	document.body.onmouseover = function() {
		if (getCookie('passpoint') == false) {
			setCookie('passpoint', 1);
			addPoint(location.href)
		}
	}
}
