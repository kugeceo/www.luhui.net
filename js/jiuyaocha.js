...
function get_wubi(){
	...
	$.ajax({
		type: "post",
		cache:false,
		url: url+'/act/wubi.php',
		data: "action=wubi&chinese="+encodeURI(chinese),
		timeout:300000,
		success: function(data){
			$('#wubi').html(data);
		},
...
function get_meta(){
	var domain = $('#domain').val().is_domain();
	if(domain){
		$("#meta").ajaxStart(function(){$(this).html('<div class="loading">正在加载...</div>')})
		...
		$.ajax({
			...
			error: function(){
				$('#meta').html('<p class="box8">服务器繁忙，请稍后再试！</p>');
			}
		});
	}
}
